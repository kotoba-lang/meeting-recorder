(ns meeting-recorder.app-test
  (:require [cljs.test :refer [deftest is testing use-fixtures]]
            [re-frame.core :as rf]
            [re-frame.db :as rf-db]
            [meeting-recorder.app :as app]))
...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates every fact the Svelte scaffold held"
    ...
    (is (= "Meeting Recorder M33tr3c0" @(rf/subscribe [:app/title])))
    (is (= "etzhayyim-wasm-meeting-recorder-m33tr3c0" @(rf/subscribe [:app/name])))
    (is (= "etzhayyim-project-meeting-recorder" @(rf/subscribe [:app/project])))
    ...
    (is (= "cljs/src/meeting_recorder/app.cljs" @(rf/subscribe [:app/relative-path])))))
